...
{
    static string[] m_excludes = new string[] {
        "m_Script", "Typeface_Number", "Typeface_Capitals", "Typeface_Small", 
        "Typeface_Commands", "Typeface_Text", "Typeface_Unicode"
        };

    static GUIContent[] fontSets;

    static GUIContent[] labels = new GUIContent[] { new GUIContent("Number"), new GUIContent("Capitals"), new GUIContent("Small"), new GUIContent("Commands"), new GUIContent("Text"), new GUIContent("Unicode") };

    static int[] fontValues;
    ...
    public bool OnInspectorGUI_Draw () {
        ...
        if (fontSets == null) 
        {
            fontSets = new GUIContent[TEXPreference.main.fontData.Length];
            fontValues = new int[fontSets.Length];
            for (int i = 0; i < fontSets.Length; i++) {
                fontSets[i] = new GUIContent(TEXPreference.main.fontData[i].id);
                fontValues[i] = i;
            }
        }
